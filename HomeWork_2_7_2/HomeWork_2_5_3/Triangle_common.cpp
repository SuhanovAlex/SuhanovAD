#include "Triangle_common.h"
    ...
    int Triangle_common::TriangleCheckSidesAreZero(unsigned int side_a, unsigned int side_b, unsigned int side_c) {
        if ((side_a == 0) || (side_b == 0) || (side_c == 0)) {
            throw "Ошибка создания фигуры. Причина: одна из сторон равна 0";
            return 0;
        };
    };
    ...
    void Triangle_common::print_info() /*override*/ {
        Figure::print_info();
        try {
            TriangleCheckSidesAreZero(side_a, side_b, side_c);
        }
